...
import {ActivityProvidersService} from "../../services/activity-providers.service.ts";
...
export default function ActivitiesTable() {
    ...
    const [formData, setFormData] = React.useState({name: '', activityProviderId: '', parameters: {}});
    ...
    useEffect(fetchAndMap, [activityService, apService, auth?.user?.access_token]);
    ...
    const handleDelete = () => {
        ...
        activityService.delete(deleteTarget.id, token())
            .then(() => fetchAndMap())
    ...
    const handleNext = () => {
        if (formData.activityProviderId) {
            apService.getConfigInterfaceUrl(formData.activityProviderId, token())
                .then(url => {
                    setIframeUrl(window.location.origin + '/config-interface?interfaceUrl=' + encodeURIComponent(url));
    ...
    const handleAdd = () => {
        apService
            ...
            .then(params => {
                const iframeWindow = document.getElementById('configInterfaceIframe') as HTMLIFrameElement;
                let parameters = {};
                for (const param of params) {
                    const val = iframeWindow?.contentDocument?.getElementById(param) as HTMLInputElement;
            ...
            .catch((e: any) => handleError(`Failed to add the activity: ${e.message}`));
    };
    ...
    const isNextDisabled = !formData.name.trim() || !formData.activityProviderId.trim();
                    ...
                    <TableBody>
                        {activityList.map((row) => (
                            ...
                            >
                                ...
                                <TableCell>{new Date(row.createdAt).toLocaleString('pt-pt')}</TableCell>
                                <TableCell>{row.createdBy}</TableCell>
                                <TableCell>{new Date(row.updatedAt).toLocaleString('pt-pt')}</TableCell>
                    ...
                    {!iframeVisible && (
                        <>
                            ...
                            <TextField
                                ...
                                onChange={(e) => setFormData({...formData, activityProviderId: e.target.value})}
                            >
                                {apList.map((ap) => (
                    ...
                    {iframeVisible && (
                        <>
                            <iframe
                                ...
                                style={{width: '100%', height: '75%', border: '1px solid #ccc'}}
                            />
                            <Button
                                variant="contained"
                                color="primary"
