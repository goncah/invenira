...
import {IAPsService} from "../../services/iaps.service.ts";
...
export default function IAPActivitiesTable({iapId, activityIds}: IAPActivitiesTableProps) {
    ...
    const fetchAndMap = () => {
        ...
        activityService
            ...
            .then((activities) => setIapActivityList(activities.filter(a => activityIds?.includes(a._id))))
            .catch(() => handleError("Failed to load IAP activities."));
    };

    useEffect(fetchAndMap, [activityIds, activityService, auth?.user?.access_token]);
    ...
    const handleRemove = () => {
        ...
        iapService.removeActivity(iapId!, removeTarget.id, token())
            .then(() => activityIds = activityIds?.filter((i) => i !== removeTarget.id))
            .then(() => fetchAndMap())
